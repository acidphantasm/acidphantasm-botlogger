...
export class CustomDynamicRouterHooks
{
    ...
    private logBotGeneration(outputJSON: any):void
    {
        ...
        {
            this.botLogger.log(Logging.ERR, "QB Bot Generation LogOutput failed.", `${err.stack}`);
        }
        const timeTaken = performance.now() - start;
        this.botLogger.log(Logging.DEBUG, `${timeTaken.toFixed(2)}ms for logging QB bot ${botDetails.role} | Name: ${botDetails.name}`);
    }

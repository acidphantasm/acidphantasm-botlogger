...
export class CustomStaticRouterHooks
{
    ...
    private logBotGeneration(outputJSON: any): void
    {
        ...
        {
            this.botLogger.log(Logging.ERR, "Bot Generation LogOutput failed.", `${err.stack}`);
        }
        const timeTaken = performance.now() - start;
        this.botLogger.log(Logging.DEBUG, `${timeTaken.toFixed(2)}ms for logging bot ${botDetails.role} | Name: ${botDetails.name}`);
    }
